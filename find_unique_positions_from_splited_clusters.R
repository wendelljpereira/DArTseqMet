    ...
    if( enzime1 %in% unique( cluster_with_overlaps$V4 ) ){
        ...
        if( unique(cluster_with_overlaps$V6) == "+" ){
            ...
            if(enzime1 %in% unique(new_positions$V4) & enzime2 %in% unique(new_positions$V4)){
                ...
                new_positions$V2 <- max(new_positions[new_positions$V4 == enzime1 , 2])
            ...
            write.table( new_positions_frags_checked,
                         file = paste('./',"good_clusters",'/', strsplit(files[i], "./")[[1]][[2]], sep = ""), 
                         col.names = F, 
                         row.names = F, 
                         quote = F, 
                         sep = "\t" )
            ...
            pos_pstI <- min(cluster_with_overlaps[cluster_with_overlaps$V4 == enzime1, 3])
            ...
            if( enzime1 %in% unique(new_positions$V4) & enzime2 %in% unique(new_positions$V4) ) {
                ...
                new_positions$V3 <- min(new_positions[new_positions$V4 == enzime1, 3])
                new_positions_frags <- new_positions[new_positions$V4 == enzime2, ]
